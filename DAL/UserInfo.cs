    ...
    class UserInfo
    {
        ...
        public Model.UserInfo GetModel(string UserName)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                ...
                return cn.QueryFirstOrDefault<Model.UserInfo>(sql);
            }
        }
        ...
        public int Update(Model.UserInfo user)
        {
            using (IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "update userinfo set password=@passWord,qq=@qq,email=@email,type=@type,userimg=@yserImg where username=@userName;";
                return cn.Execute(sql, user);
            }
        }
